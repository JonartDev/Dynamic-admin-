<div class="main-content" id="mainList">
    ...
    <div class="row">
            ...
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-row justify-content-between">
                        ...
                        <button data-bs-toggle="modal" data-bs-backdrop="static" data-bs-keyboard="false"
                            data-bs-target="#addnewSlugsModal" class="btn bg-gradient-primary btn-sm mb-0"
                            id="addnew" type="button">
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="row ">
                        ...
                        <div class="col-md-4 p-2 pr-4">
                            <input class="inputSearch" type="text" wire:model="search"
                                placeholder="{{ __('translate.search') }}..." id="email" name="email"
                    ...
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                                @if (count($slugsData) > 0)
                                    @foreach ($slugsData as $slugs)
                                        <tr>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $slugs->name }}</p>
                                            </td>
                                            <td class="text-center">
                                            ...
                                            <td class="text-center">
                                                ...
                                                <div x-data="{ copyUrl: '{{ url('/sl/' . $slugs->slug) }}', notification: false }">
                                                    <p>
                                                        <button
                                                            @click="copyToClipboard(copyUrl); notification = true; setTimeout(() => notification = false, 2000)">
                                                            {{ url('/sl/' . $slugs->slug) }}
                                                        </button>
                                                    </p>

                                                    <!-- Notification -->
                                                    <div x-show="notification" class="notification">
                                                        Copied to clipboard!
                                                    </div>
                                            ...
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $slugs->code }}</p>
                                            </td>

                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0"
                                                    style="color: <?php echo $slugs->status == '1' ? 'green' : 'red'; ?>">
                                                    {{ $slugs->status == '1' ? __('translate.active') : __('translate.inactive') }}
                                                </p>
                                            </td>
                                            <td class="text-center">
                                                <button type="button" class="mx-1" data-bs-toggle="modal"
                                                    data-bs-target="#updateSlugsModal"
                                                    wire:click="editSlug({{ $slugs->id }})">
                                                ...
                                                <button type="button" class="mx-1 text-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteSlugsModal"
                                                    wire:click="deleteSlug({{ $slugs->id }})">
        ...
        @include('livewire.modal.slug_modal')
    </div>
</div>
